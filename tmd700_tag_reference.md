...
| `e` | DEMO mode indicator | `e1`=demo mode active; `e0`=normal (both confirmed) | Body sends `e1` right after the boot state dump when the head requested demo mode (head sends `0xA7` = `[F]` held at power-on). Lowercase — distinct from any uppercase tag. The demo itself (graphics screen, DIM cycling, video-invert) runs **entirely in the head** with no further tag exchange. Confirmed 2026-07-04 via P2 MITM. |
